...
import { validateAddressUpdate, addressUpdateSchema } from '@/lib/validation';
import { z } from 'zod';
...
export const PUT = withAuth(async (request, user, { params }) => {
  try {
    ...
    const validation = validateAddressUpdate(payload);
    if (validation.error) {
      return handleApiError(validation.error, 'Update Address');
    }
    const data = validation.data as z.infer<typeof addressUpdateSchema>;
