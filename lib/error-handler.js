...
export class StockError extends Error {
  constructor(message, productId = null, deliveryId = null) {
    super(message);
 ...
 */
export function handleApiError(error, context = 'API', metadata = {}) {
 ...
 */
export function handleValidationErrors(errors, context = 'Validation') {
 ...
 */
export function handleDatabaseError(dbError, context = 'Database') {
 ...
 */
export function logApiRequest(request, user = null, duration = 0, statusCode = 200) {
  const logData = {
    timestamp: new Date().toISOString(),
    level: 'INFO',
 ...
 */
export function withErrorHandling(handler, context = 'API') {
  return async (...args) => {
    try {
 ...
 */
export function createSuccessResponse(data, meta = null, status = 200) {
  const response = {
    success: true,
    data,
    timestamp: new Date().toISOString()
